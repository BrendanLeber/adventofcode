...
* Day 5: How About a Nice Game of Chess?
  ...
  at a time by finding the [[https://en.wikipedia.org/wiki/MD5][MD5]] hash of some Door ID (your puzzle
  input) and an increasing integer index (starting with =0=).

  A hash indicates the *next character* in the password if its
  [[https://en.wikipedia.org/wiki/Hexadecimal][hexadecimal]] representation starts with *five zeroes*.  If it does, the
