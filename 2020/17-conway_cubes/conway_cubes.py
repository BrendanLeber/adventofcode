...
def count_neighbors(world: World, pt: Point) -> int:
    ...
    for offset in NEIGHBORS:
        off: Point = Point(pt.x + offset.x, pt.y + offset.y, pt.z + offset.z)
        if off in world:
            neighbors += 1
    return neighbors
...
def get_world_cube(world: World):
    ...
    return (Extent(min_x, max_x + 1), Extent(min_y, max_y + 1), Extent(min_z, max_z + 1))
