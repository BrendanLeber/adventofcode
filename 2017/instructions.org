...
** Part One
   ...
   scanning area for the scanner within it, written as =depth: range=.
   Each layer has a thickness of exactly =1=.  A layer at depth =0=
   ...
   =2=), a third layer which begins at depth =4= (with range =4=), and
   a fourth layer which begins at depth =6= (also with range =4=).
   ...
   top, and repeats.  A security scanner takes *one picosecond* to
   move one step.  Drawing scanners as =S=, the first few picoseconds
   look like this:
   ...
   it moves at *one layer per picosecond*.  Each picosecond, the
   packet moves one layer forward (its first move takes it into layer
   0), and then the scanners move one step.  If there is a scanner at
   the top of the layer *as your packet enters it*, you are *caught*.
   (If a scanner moves into the top of its layer while you are there,
   you are *not* caught: it doesn't have time to notice you before you
