...
class LineSegment:
    ...
    Methods
    -------
    standard_form : Tuple[int, int, int]
        The line represented in standard form.
    ...
    def standard_form(self) -> Tuple[int, int, int]:
        """Convert the line segment into standard form (Ax + By = C)."""
        a = self.q.y - self.p.y
        b = self.p.x - self.q.x
...
if __name__ == "__main__":
    ...
    class LineSegmentUnitTests(unittest.TestCase):
        def test_standard_form(self):
            segment = LineSegment(Point(3, 2), Point(2, 6))
            self.assertEqual(segment.standard_form(), (4, 1, 14))

            segment = LineSegment(Point(0, 1), Point(2, 4))
            self.assertEqual(segment.standard_form(), (3, -2, -2))
