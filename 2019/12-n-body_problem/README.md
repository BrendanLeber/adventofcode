...
[big distracting red spot][1], extreme [radiation][2], and a [whole lot of
moons][3] swirling around.  You decide to start by tracking the four largest
moons: *Io*, *Europa*, *Ganymede*, and *Callisto*.

[1]: https://en.wikipedia.org/wiki/Great_Red_Spot
[2]: https://en.wikipedia.org/wiki/Magnetosphere_of_Jupiter
[3]: https://en.wikipedia.org/wiki/Moons_of_Jupiter#List
...
`z`), the velocity of each moon changes by *exactly +1 or -1* to pull the moons
...
(because `5 > 3`) and Callisto's `x` velocity *changes by -1* (because `3 <
...
energy*.  A moon's *potential energy* is the sum of the [absolute values][4] of
its `x`, `y`, and `z` position coordinates.  A moon's *kinetic energy* is the
sum of the absolute values of its velocity coordinates.  Below, each line shows
the calculations for a moon's potential energy (`pot`), kinetic energy (`kin`),
and total energy:

[4]: https://en.wikipedia.org/wiki/Absolute_value
